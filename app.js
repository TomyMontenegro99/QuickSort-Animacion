  ...
  for (const step of steps) {
    switch (step.type) {
      ...
      case 'compare': {
        ...
        updateStatus(
          `Paso ${stepNumber}: Comparando ${step.value} con el pivote ${step.pivotValue}. ${
            step.result ? 'Es menor o igual, se mueve a la izquierda.' : 'Permanece a la derecha.'
          }`
        );
        await pause(1);
      ...
      case 'swap-or-accept': {
        ...
        if (from === to) {
          ...
          updateStatus(
            `Paso ${stepNumber}: Intercambiamos ${state.numbers[from]} con ${state.numbers[to]} para llevarlo al sub-arreglo izquierdo.`
          );
          await pause(0.45);
          swapState(from, to);
          await pause(0.9);
